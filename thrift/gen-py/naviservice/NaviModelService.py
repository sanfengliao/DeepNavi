...
class Client(Iface):
    ...
    def predict(self, model):
        ...
        """
        self.send_predict(model)
        return self.recv_predict()

    def send_predict(self, model):
...
class Processor(Iface, TProcessor):
    ...
    def process_predict(self, seqid, iprot, oprot):
        ...
        result = predict_result()
        try:
            result.success = self._handler.predict(args.model)
...
predict_args.thrift_spec = (
    None,  # 0
    (1, TType.STRUCT, 'model', [NaviModel, None], None, ),  # 1
)
...
predict_result.thrift_spec = (
    (0, TType.STRUCT, 'success', [LocationResult, None], None, ),  # 0
)
